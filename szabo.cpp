...
int main(int argc, char const *argv[]) {
  ...
  float mu = 1, tau = 1, Req = 5/6, R0 = 1, Fadh = 0.75, Frep = 30, v0 = 1;
  float noise = 0.6, rhoNorm = 0.3; //tunable parameters
  float L = R0 * sqrt(N_particles/(2*rhoNorm)); //rhoMax = 2
...
//main solver loop
  for(int k = 0;k<N_steps;k++){
    for (int i = 0;i<N_particles;i++){
      float interaxn[2] = {0,0}; //interaction force vector for ith particle
      for (int j = 0;j<N_particles;j++){
        if (i==j)
          continue;
        float dx = solX[k][i] - solX[k][j];//dij vector = (dx,dy)
